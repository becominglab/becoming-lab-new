...
export default function MissionPage() {
  ...
  const handleMissionTap = useCallback(
    (mission: Mission) => {
      ...
      if (mission.type === 'quiz' && mission.subject) {
        router.push(
          `/nobishiro/quiz?subject=${encodeURIComponent(mission.subject)}&count=${mission.questionCount ?? 3}&missionId=${encodeURIComponent(mission.id)}`
        );
      } else if (mission.type === 'reflection') {
        router.push(`/nobishiro/review?missionId=${encodeURIComponent(mission.id)}`);
      }
    },
    [router]
  );
  ...
  const handleExtraPractice = useCallback(() => {
    ...
    router.push(`/nobishiro/quiz?subject=${encodeURIComponent(subject)}&count=3`);
  }, [router, state]);

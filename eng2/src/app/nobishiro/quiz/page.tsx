...
function QuizInner() {
    ...
    (choice: string) => {
      ...
      setTimeout(() => {
        if (currentIndex + 1 >= total) {
          ...
          router.push(
            `/nobishiro/result?correct=${finalCorrect}&total=${total}&subject=${encodeURIComponent(subject)}`
          );
        } else {
  ...
  const handleNext = useCallback(() => {
    ...
    if (currentIndex + 1 >= total) {
      ...
      router.push(
        `/nobishiro/result?correct=${correctCount}&total=${total}&subject=${encodeURIComponent(subject)}`
      );
    } else {
      setCurrentIndex((i) => i + 1);
      setSelectedAnswer(null);
